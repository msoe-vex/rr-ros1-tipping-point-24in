...
AutonManagerNode::AutonManagerNode(NodeManager* node_manager, IDriveNode* drive_node, IClawNode* claw_node, ILiftNode* lift_node) : 
        Node(node_manager, 50),
        m_drive_node(drive_node),
        m_claw_node(claw_node),
        m_lift_node(lift_node) {
    m_basic_auton = new BasicAuton(m_drive_node, m_claw_node, m_lift_node);
