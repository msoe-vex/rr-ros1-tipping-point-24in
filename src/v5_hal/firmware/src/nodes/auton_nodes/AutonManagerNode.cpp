...
AutonManagerNode::AutonManagerNode(NodeManager* node_manager, IDriveNode* drive_node, ADIEncoderNode* y_odom_encoder, InertialSensorNode* inertial_sensor) : 
        Node(node_manager, 50),
        m_drive_node(drive_node),
        m_y_odom_encoder(y_odom_encoder),
        m_inertial_sensor(inertial_sensor) {
    m_basic_auton = new BasicAuton(m_drive_node, m_y_odom_encoder, m_inertial_sensor);
